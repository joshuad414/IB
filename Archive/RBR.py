...
class RBR:
    def __init__(self, symbol, option_date, strike_price, qty_buy):
        self.symbol = symbol
        self.option_date = option_date
        self.strike_price = strike_price
        self.qty_buy = qty_buy
    ...
    def option_info(self):
        x = Option(self.symbol, self.option_date, self.strike_price, 'SMART', '100', 'USD')
        return x
    ...
    def stock_data(self):
        ib.reqMktData(self.stock_info(), '', False, False)

    def option_data(self):
        ib.reqMktData(self.option_info(), '', False, False)
    ...
    def on_pending_ticker(self, ticker):
        ...
        last_price_stock = self.stock_data().last  # get last ticker price
        last_price_option = self.option_data().last

        # Buy
        if last_price_stock > base_high and watching == True and count == 0 and trades == 0 and \
                ((t1 - t0) > 300 or (t1 - t0) == 0):
            ib.placeOrder(self.option_info(), MarketOrder('BUY', self.qty_buy))
            t0 = time.time()
            ib.sleep(3)
            count += self.qty_buy
            time_executed = datetime.datetime.now().strftime("%H:%M:%S")
            executed_buy_price = self.get_executed_price()
            exec_trade_list.append(executed_buy_price)
            last_price_list.append(last_price_stock)
            qty_list.append(self.qty_buy)
            ...
            buy_data = pd.DataFrame({'Executed Trade Price': exec_trade_list, 'Price of Stock': last_price_list,
                                     ...
                                     'Time': time_list})
            buy_data.to_csv('Executed_Trades/trade_data_' + self.symbol + '.csv')
        ...
        # Sell 1
            ...
            ib.placeOrder(self.option_info(), MarketOrder('SELL', qty_sold))
            count -= qty_sold
            trades = 1
            executed_trade_price = self.option_data().last
            time_executed = datetime.datetime.now().strftime("%H:%M:%S")
        ...
        # Sell 5
            ...
            trade_data = pd.DataFrame({'Executed Trade Price': exec_trade_list, 'Price of Stock': last_price_list,
                                       ...
                                       'Basing High': basing_high_list,
                                       'Time': time_list})
            trade_data.to_csv('Executed_Trades/trade_data_' + self.symbol + '.csv')
            print(qty_sold, 'SELL @', executed_trade_price, time_executed, trades)

        # Sell remaining
        elif last_price_stock < basing_open and count != 0:
            ib.placeOrder(self.option_info(), MarketOrder('SELL', count))
            t1 = time.time()
            count = 0
            trades = 0
            executed_trade_price = self.option_data().last
...
'''Run: symbol, option date, strike price, buy qty'''
ib = IB()
ib.connect('127.0.0.1', 7497, clientId=1)

s1 = RBR("MSFT", "20210409", 243, 50)
