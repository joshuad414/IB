...
bars = ib.reqHistoricalData(Stock('SPY', 'SMART', 'USD'), endDateTime='', durationStr='1 D',
                         barSizeSetting='5 mins', whatToShow='MIDPOINT', useRTH=True, keepUpToDate=True)
df = util.df(bars)
...
for x in stock_list:
    ...
    bars = ib.reqHistoricalData(stock, endDateTime='', durationStr='1 D', barSizeSetting='5 mins',
    ...
    df2['bar2'] = ((df2['bar1'].values == True) & (df2['rally'].values == True) & (df2['open_close'].shift(periods=2).values > 0)) * 1
    df2['bar3'] = ((df2['bar2'].values == True) & (df2['rally'].values == True) & (df2['open_close'].shift(periods=3).values > 0)) * 1
    df2['bar4'] = ((df2['bar3'].values == True) & (df2['rally'].values == True) & (df2['open_close'].shift(periods=4).values > 0)) * 1
    df2['bar5'] = ((df2['bar4'].values == True) & (df2['rally'].values == True) & (df2['open_close'].shift(periods=5).values > 0)) * 1
    df2['bar'] = (df2['bar1'].values + df2['bar2'].values + df2['bar3'].values + df2['bar4'].values + df2['bar5'].values)
    df = df.append(df2)
